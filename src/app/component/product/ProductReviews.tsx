    ...
    const fetchReviews = async () => {
      ...
      try {
        ...
        const transformedReviews: Review[] = apiReviews.map((review: ProductReview) => ({
          ...
          verified: true,
          // Always include reviewerName if user information is available
          reviewerName: review.user ? `${review.user.firstName} ${review.user.lastName}` : undefined
        }));
                      ...
                      <div className="flex flex-col gap-3">
                        ...
                        <div className="flex items-start justify-between">
                          <div>
                            ...
                            {/* Always show reviewer name if it exists */}
                            {review.reviewerName ? (
                              <p className="text-sm font-medium text-gray-900">
                                {review.reviewerName}
                              </p>
                            ) : (
                              <div className="h-5"></div> // Empty div to maintain spacing
                            )}
                          </div>
